            ...
            if source is not None and target is not None:
                ...
                if points is not None:
                    for pt in points.findall("mxPoint"):
                        arrow["waypoints"].append((float(pt.get("x")), float(pt.get("y"))))
...
def catmull_rom_to_bezier(p0, p1, p2, p3, tension=0.5):
    """Convert 4 Catmull-Rom points to 4 Bezier control points."""
    d1 = np.linalg.norm(np.subtract(p1, p0))
    d2 = np.linalg.norm(np.subtract(p2, p1))
    d3 = np.linalg.norm(np.subtract(p3, p2))
...
def plot_arrows(arrows):
    ...
    for arrow in arrows:
        ...
        # Extend points for Catmull-Rom (phantom points at start/end)
        extended = [points[0]] * 2 + points + [points[-1]] * 2
        
        # Generate Bezier segments for each interval
        for i in range(1, len(extended) - 2):
            ...
            curve = np.array([(1 - t)**3 * bezier_ctrl[0][0] + 3 * (1 - t)**2 * t * bezier_ctrl[1][0] + 
                              3 * (1 - t) * t**2 * bezier_ctrl[2][0] + t**3 * bezier_ctrl[3][0],
                              (1 - t)**3 * bezier_ctrl[0][1] + 3 * (1 - t)**2 * t * bezier_ctrl[1][1] + 
                              3 * (1 - t) * t**2 * bezier_ctrl[2][1] + t**3 * bezier_ctrl[3][1]])
            ax.plot(curve[0], curve[1], color=color, lw=2)
    
    ax.set_xlim(0, 800)
    ax.set_ylim(0, 700)
    ax.set_aspect('equal')
    plt.gca().invert_yaxis()  # Match draw.io's coordinate system
    plt.show()

# Sample XML content (same as your input)
