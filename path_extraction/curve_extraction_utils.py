...
from utils.split_contour import extract_curve_from_mask_v1
...
def extract_curve_from_mask(image: cv2.typing.MatLike):
    ...
    paths = extract_curve_from_mask_v1(image)
    ...
    while len(paths) > 0:
        for i, path in enumerate(paths):
            ...
            #     cv2.line(image, tuple(final_path[j]), tuple(final_path[j+1]), (255, 255, 255), 2)

            # # draw a circle at the start and end of the final path
            # cv2.circle(image, tuple(final_path[0]), 5, (0, 0, 255), -1)
            # cv2.circle(image, tuple(final_path[-1]), 5, (0, 0, 255), -1)            

            # cv2.imshow("new path - ", image)
            # cv2.waitKey(0)            
