...
def process_arrow_contours(contours, max_gap=10):
    # Load binary image (arrow in white, background black)
    ...
    # Step 1: Extract simplified polylines and their endpoints
    polylines = []
    endpoints = []  # List of tuples: (start_point, end_point, is_closed)
    for cnt in contours:
        # Approximate contour as a polyline (reduce points)
        ...
        is_closed = np.allclose(poly[0], poly[-1], atol=2)
        if is_closed:
            # Split closed contour into open path by finding farthest points
            distances = cdist(poly, poly)
            i1, i2 = np.unravel_index(np.argmax(distances), distances.shape)
            poly = np.roll(poly, -i1, axis=0)
            poly = poly[:i2 - i1 + 1]  # Take the longer segment
        polylines.append(poly)
        endpoints.append((poly[0], poly[-1]))

    # Step 2: Build adjacency list using endpoint proximity
    graph = {}
    ...
    for i in range(len(polylines)):
        for j in range(i + 1, len(polylines)):
            # Check connections between all endpoints of i and j
            for pt_i in [endpoints[i][0], endpoints[i][1]]:
                for pt_j in [endpoints[j][0], endpoints[j][1]]:
                    if np.linalg.norm(pt_i - pt_j) <= max_gap:
                        graph[i].append(j)
                        graph[j].append(i)

    # Step 3: Find the longest non-branching path using greedy traversal
    visited = set()
    best_path = []

    def extend_path(current_path, current_node):
        nonlocal best_path
        ...
        for neighbor in graph[current_node]:
            if neighbor not in visited:
                ...
                extend_path(current_path + [neighbor], neighbor)
                visited.remove(neighbor)

    for start_node in range(len(polylines)):
        if start_node not in visited:
            visited.add(start_node)
            extend_path([start_node], start_node)
            visited.remove(start_node)

    # Step 4: Merge polylines in the best path with correct order
    if not best_path:
        return None
    ...
    for idx in best_path:
        ...
        if current_end is None:
            # Initialize with the entire polyline
            ordered_points.extend(poly)
            current_end = end
        else:
            # Check which endpoint connects to the current_end
            dist_to_start = np.linalg.norm(start - current_end)
            dist_to_end = np.linalg.norm(end - current_end)

            if dist_to_start < dist_to_end:
                # Append forward direction
                ordered_points.extend(poly)
                current_end = end
            else:
                # Append reversed direction
                ordered_points.extend(poly[::-1])
                current_end = start

    # Step 5: Remove duplicates and ensure single path
    # Use numpy to remove consecutive duplicates
    arr = np.array(ordered_points)
    mask = np.concatenate(([True], np.any(np.diff(arr, axis=0) != 0, axis=1)))
    unique_ordered = arr[mask]

    # Step 6: Fit spline to the ordered points
    t = np.arange(len(unique_ordered))
    cs_x = CubicSpline(t, unique_ordered[:, 0])
    cs_y = CubicSpline(t, unique_ordered[:, 1])
    t_fine = np.linspace(0, len(unique_ordered)-1, 1000)
    x_fit = cs_x(t_fine)
    y_fit = cs_y(t_fine)
...
#     curve = process_arrow_contours("arrow.png", max_gap=15)
